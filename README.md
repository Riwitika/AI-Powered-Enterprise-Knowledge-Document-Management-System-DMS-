...
The AI assistant uses a real RAG pipeline — every response is generated live by Google Gemini using document context retrieved from locally computed Sentence Transformer embeddings.
...
| **PDF/PPTX/XLSX Editing** | These formats are view-only in the browser. Text is extracted for AI search but the original file cannot be edited in-place |
| **Image Upload** | Image files (PNG/JPG) are supported in seed data and have viewers, but the upload endpoint currently restricts to document formats (PDF, DOCX, XLSX, PPTX, TXT) |
...
| **AI Streaming** | AI responses arrive as a single payload. The typing animation in the chat UI is a client-side effect applied after the full response is received |
...
| Module | Status |
|---|---|
| 🟢 Core DMS (CRUD, folders, versions) | Complete |
| 🟢 Authentication & RBAC | Complete |
| 🟢 Document Management & Approval Workflow | Complete |
| 🟢 Rich Tiptap Editor with Pagination | Complete |
| 🟢 AI Document Assistant (Gemini + RAG) | Complete |
| 🟢 Upload & Document Processing Pipeline | Complete |
| 🟢 Backend Persistence (SQLite + PostgreSQL ready) | Complete |
| 🟢 CI Pipeline (GitHub Actions) | Complete |
| 🟡 Image Upload Support | Partial (viewers exist, upload restricted) |
| 🟡 DOCX Formatting Fidelity | Partial (basic formatting preserved) |
